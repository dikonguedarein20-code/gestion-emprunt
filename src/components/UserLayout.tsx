...
const menuItems: Array<{ path: string; icon: React.ComponentType<React.SVGProps<SVGSVGElement>>; labelKey: TranslationKey }> = [
  { path: '/user', icon: Package, labelKey: 'availableMaterials' },
  { path: '/user/request', icon: FileText, labelKey: 'makeRequest' },
  { path: '/user/report', icon: Wrench, labelKey: 'reportBreakdown' },
];
        ...
        <nav className="mt-6 px-3">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`w-5 h-5 transition-colors ${isActive ? 'text-emerald-400' : ''}`}
                />
